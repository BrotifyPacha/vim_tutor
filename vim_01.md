...
Для перемещения в конец строки используется символ `$`. Почему именно доллар?
Знак доллара в регулярных выражениях используется для обозначения конца строки.

Допустим у вас есть строка с индентацией, и вам необходимо *"прыгнуть"* в
начало строки, но именно на первый не пустой символ, для этого используйте `^`.
...
изменить. Здесь нам на помощь приходят текстовые объекты - это отрезки текста
которые Vim понимает.
