...
#### :help - документация внутри Vim

#### Выполнение команд

Хорошо, мы определились какую команду хотим выполнить, нажали `:`, ввели,
допустим, `set number`, а что теперь? А теперь нужно лишь нажать `Enter`.

Полностью наша команда выглядит так - `:set number<CR>`, стоп, но где `Enter`,
и что это за `<CR>`?

#### Синтаксис обозначения сочетаний клавиш и специальных клавиш в Vim

Все специальные клавиши и сочетания клавиш в Vim обозначаются через
`<` и `>` скобки.

Итак почему `<CR>`? Это сокращение от Carriage Return (возвращение каретки),
это пошло ещё от печатных машинок, так там называлась клавиша ныне извастная нам
как Enter.
...
Я специально написал не все особые клавиши, давайте разберём сочетания клавиш
и я покажу вам способ узнать как Vim под капотом интерпритирует любую клавишу.

Итак, как я сказал ранее, сочетания клавиш также обозначаются через прямоуголь-
ные скобки, но так как клавиш там несколько клавиш, то они пишутся через символ
минуса `-`.
...
Как и обещал рассказываю про то как узнать как Vim интерпритирует любую
клавишу, ключем к этой тайне является сочетание клавиш `<C-k><Любая клавиша>`,
используйте его в *командном режиме* или в *режиме ввода*

Давайте прочитаем его вместе - первае сочитание клавиш (`<C-k>`) это комбинация
`Ctrl` и `k`, а затем любая интересующая нас клавиша.
...
Есть два вида **действий** - те что манипулируют над символом под курсором и те
...
3 символа, включая символ под курсором на символ A, `3x` - удалит три символа, а
...
* `ap` - текущий параграф, и пустая строка перед ним (**A**round **P**aragraph,
  вокруг параграфа);
* `it` - текст внутри текущего HTML тега;
* `at` - текст внутри текущего HTML тега, а также сам тег;
* `i(`, `i[`, `i{`, `i<`, `` i` ``, `i'`, `i"` - весь
...
Для того чтобы вставить текст после курсора используйте клавишу `p`,
при желании текст можно вставить и до курсора, для этого следует
использовать клавишу `P`.

## Регистры

<!-- TODO: написать про регистры -->

## Перемещение по файлу

Перемещение по файлу осуществляется также *движенями*. В разделе
[Normal Mode](#normal-mode-(обычный-режим)) мы с вами рассмотрели движения в
масштабе строк, сейчас мы рассмотрим их в масштабе файла.

Перемещение в начало файла осуществляется нажатием `gg`. Для того чтобы
перемеситься в конец файла используйте `G`.

Обе эти команды принимают *числа*, они использутся чтобы вместо начала или
конца файла перемеситься на определенную строку. Так `13gg` и `13G` перемесят
наш курсор на 13 строку файла. Здесь как раз полезна настройка `:set number`.

Иногда нам нужно осуществить движение не относительно всего файла, а лишь
отображаемой области, для этого используйте клавиши `H`, `M` и `L`
(**H**igh, **M**iddle и **L**ow, что в переводе с английского, "Верх",
"Середина" и "Низ" соответственно)

Клавиша `H` перемещает курсор на первую строку отображаемой области, клавиша
`M` перемещает курсор на строку что посередине, а клавиша `L` - на последнюю
отображаемую.

### Прокрутка экрана

Прокрутка экрана в Vim осуществляется с помощью сочетаний клавиш
`<C-e>`, `<C-y>`, `<C-d>`, `<C-y>`.

Важно понимать что в отличии от обычных редакторов, курсор в Vim не может
покинуть отображаемую область файла, поэтому при прокрутке, мы будем так,
так сказать, тащить курсор за нами.

## Работа с несколькими файлами

### Буферы, Окна и Вкладки (Buffers, Windows and Tabs)

Для работы с несколькими файлами, сначала нужно понять как сам Vim работает с
ними.

Когда вы открываете какой либо файл в Vim, для него создаётся **Буфер
(Buffer)**, этот Буфер привязывается к **Окну (Window)** и это Окно помещается в
открытой **Вкладке (Tab)**

Другими словами:

**Буфер (Buffer)** - редактируемый файл.

**Окно (Window)** - отображение буфера.

**Вкладка (Tab)** - набор окон. Отображаться может только одна.

Если вы пользовались Notepad++, VSCode или какой либо IDE то для вас Вкладка
это один файл, учтите сразу что в Vim это не так. Вкладка Vim это любое кол-во
окон.

Каждый сам для себя определяет смысл вкладки. Я испльзую их для работы с
несколькими проектами - в одной вкладке один проект, во второй - другой, и т.д.

К примеру вот состояние Vim при обычном запуске. Открывается **Вкладка**
содержащая одно **Окно**, показывающее пустой **Буфер**.
...
<!-- TODO: написать про буферы -->

#### Окна

<!-- TODO: написать про окна -->

#### Вкладки

<!-- TODO: написать про вкладки -->
